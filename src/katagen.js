...
const printVersion = () => console.log(`katagen - version ${version}`);
...
if (args.h || args.help) {
  printVersion();
  console.log(`Usage: katagen [options]${EOL}${EOL}All flags are optional.${EOL}  -h, --help        print this message${EOL}  -n, --new-day     force to create a new day with the kata${EOL}  -v, --version     print program version`);
  process.exit();
}
...
buildConfig()
  .then(async (config) => {
    const { kataLink } = await inquirer.prompt([{
      ...
      default: '',
      validate: (link) => (link === '' || link.startsWith('https://www.codewars.com/kata/') ? true : 'Please provide a kata link from Codewars or leave empty to build README'),
    }]);
    ...
    if (kataLink !== '') {
      ...
      if (kata) {
        ...
        if (!Object.prototype.hasOwnProperty.call(config.katas, slug)) {
          ...
          const nextDay = (args.n || args['new-day']) ? new Date() : new Date(lastTime + 86400000);
          ...
          if (kyuList.includes(kataKyu)) {
            ...
            readme = (append.length === 0)
              ? `${readme}${EOL}${EOL}${kyuText}`
              : readme.replace(`# ${append[0]} kyu`, `${kyuText}${EOL}${EOL}# ${append[0]} kyu`);
          }
      ...
      console.warn('kata link is empty, only generating readme...');
    }

    const { overwrite } = await inquirer.prompt([{
      type: 'confirm',
      name: 'overwrite',
      message: 'Do you want overwrite your current README?',
      default: true,
    }]);
    if (overwrite) {
      // generate readme
      let readme = `# what's this?${EOL}${EOL}this repo contains solutions to various katas from codewars.${EOL}${EOL}kata links can be found in each day's readme.${EOL}${EOL}###### generated by [katagen](https://www.npmjs.com/package/katagen)${EOL}`;
      if (config.tok) {
        readme += `${EOL}# table of katas${EOL}${EOL}`;
            ...
            sections.forEach((section) => {
              ...
              if (kata) {
                ...
                const link = kata
                  .match(/\(.*\)/g)[0]
                  .slice(1, -1)
                  .replace(/(.*)*/, `./${dayName}/${slug}${config.katas[slug]}`);
                readme += `|[${slug}](${link})`;
              } else {
                readme += '|';
              }
